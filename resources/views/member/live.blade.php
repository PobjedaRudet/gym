...
<style>
  ...
  .refresh-btn {
    ...
    color: #FF375F; border-radius: 10px; padding: 8px 16px;
    font-size: 13px; font-weight: 600; cursor: pointer;
    display: inline-flex; align-items: center; gap: 6px;
    transition: all 0.2s; text-decoration: none;
  }
  .refresh-btn:hover {
    background: rgba(255,55,95,0.15); color: #FF6482;
  }
  ...
  <div class="row g-4">
    ...
    <div class="col-12 col-md-6">
      <div class="live-card">
        ...
        <div class="live-card-count" style="color:#5AC8FA;">{{ $gymMembers->count() }}</div>
        <div class="live-card-label">Gym</div>
        <div class="live-card-status" style="background:{{ $gymMembers->count() > 0 ? 'rgba(48,209,88,0.08)' : 'rgba(0,0,0,0.04)' }};color:{{ $gymMembers->count() > 0 ? '#30D158' : '#999' }};">
    ...
    <div class="col-12 col-md-6">
      <div class="live-card">
        <div class="live-card-icon" style="background:linear-gradient(135deg,#FF375F,#FF6482);">
          <svg width="28" height="28" fill="none" stroke="#fff" stroke-width="2" viewBox="0 0 24 24"><path d="M6.5 6.5h11M6.5 17.5h11M2 12h20M4 9v6M20 9v6M6.5 6.5v11M17.5 6.5v11"/></svg>
        </div>
        <div class="live-card-count" style="color:#FF375F;">{{ $ladiesMembers->count() }}</div>
        <div class="live-card-label">Ladies Gym</div>
        <div class="live-card-status" style="background:{{ $ladiesMembers->count() > 0 ? 'rgba(48,209,88,0.08)' : 'rgba(0,0,0,0.04)' }};color:{{ $ladiesMembers->count() > 0 ? '#30D158' : '#999' }};">
