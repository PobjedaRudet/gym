...
<h4 style="font-weight:800;color:#1C1C1E;margin-bottom:1.5rem;">
    <a href="{{ route('moderator.obavijesti') }}" style="color:#8E8E93;text-decoration:none;">Obavijesti</a>
    <span style="color:#ccc;margin:0 8px;">/</span> Nova obavijest
</h4>
...
<div class="glass-card p-4" style="max-width:700px;">
    ...
    <form method="POST" action="{{ route('moderator.obavijesti.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
            <label class="form-label" style="font-weight:600;font-size:13px;color:#555;">Naslov</label>
            <input type="text" name="naslov" class="form-control" value="{{ old('naslov') }}" required style="border-radius:12px;padding:10px 14px;">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <textarea name="sadrzaj" class="form-control" rows="5" required style="border-radius:12px;padding:10px 14px;">{{ old('sadrzaj') }}</textarea>
        </div>
        <div class="mb-4">
            ...
            <div style="border:2px dashed #ddd;border-radius:12px;padding:1.5rem;text-align:center;cursor:pointer;transition:all 0.2s;" id="dropZone" onclick="document.getElementById('slikaInput').click()">
                ...
                <div id="uploadPlaceholder">
                    ...
                    <p style="color:#8E8E93;font-size:13px;margin:8px 0 0;">Klikni ili prevuci sliku ovdje</p>
                ...
                <div id="imagePreview" style="display:none;">
                    ...
                    <p style="color:#30D158;font-size:12px;font-weight:600;margin:8px 0 0;">Slika učitana</p>
                </div>
            </div>
            @error('slika') <div style="color:#FF375F;font-size:12px;margin-top:4px;">{{ $message }}</div> @enderror
        </div>
        <div class="d-flex gap-2">
            <button type="submit" class="btn" style="background:#FF375F;color:#fff;font-weight:700;border-radius:10px;padding:10px 24px;">Kreiraj obavijest</button>
            <a href="{{ route('moderator.obavijesti') }}" class="btn" style="background:#f0f0f0;color:#555;font-weight:600;border-radius:10px;padding:10px 24px;">Odustani</a>
        </div>
    </form>
</div>
